...
$chat_room_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($max_users < 2) {
            throw new Exception("Maximum users must be at least 2.");
        }
        
        // Update chat room
        $stmt = $pdo->prepare("UPDATE chat_rooms SET 
            room_name = ?, description = ?, max_users = ?, is_private = ?, 
            updated_at = NOW() 
            WHERE chat_room_id = ?");
        
        $stmt->execute([
            $room_name, $description, $max_users, $is_private, $chat_room_id
        ]);
        ...
        $stmt = $pdo->prepare("SELECT * FROM chat_rooms WHERE chat_room_id = ?");
...
// Get chat room data if not already loaded
if (!$chat_room) {
    $stmt = $pdo->prepare("SELECT * FROM chat_rooms WHERE chat_room_id = ?");
    $stmt->execute([$chat_room_id]);
    $chat_room = $stmt->fetch();
...
<head>
    ...
    <title>Edit Chat Room - CITSA Admin</title>
                ...
                <div class="mb-4">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="chat_rooms.php">Chat Rooms</a></li>
                            <li class="breadcrumb-item active">Edit Chat Room</li>
                        </ol>
                    </nav>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0">
                            ...
                            Edit Chat Room: <?php echo htmlspecialchars($chat_room['room_name']); ?>
                        </h5>
                    </div>
                    <div class="card-body">
                        <form method="POST">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="room_name" name="room_name" 
                                       value="<?php echo htmlspecialchars($chat_room['room_name']); ?>" required>
                            </div>

                            <div class="mb-3">
                                <label for="description" class="form-label">Description</label>
                                <textarea class="form-control" id="description" name="description" rows="4"><?php echo htmlspecialchars($chat_room['description'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="max_users" name="max_users" 
                                           value="<?php echo htmlspecialchars($chat_room['max_users'] ?? 50); ?>" min="2" max="100">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <div class="form-check mt-4">
                                        <input class="form-check-input" type="checkbox" id="is_private" name="is_private" 
                                               <?php echo ($chat_room['is_private'] ?? 0) ? 'checked' : ''; ?>>
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary-custom">
                                    <i class="bi bi-save me-2"></i>Update Chat Room
                                </button>
                                <a href="chat_rooms.php" class="btn btn-outline-secondary">
                                    <i class="bi bi-arrow-left me-2"></i>Back to Chat Rooms
                                </a>
                            </div>
                        </form>
                    </div>
